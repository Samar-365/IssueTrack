...
import { effect, frame, init, sampler, storage, surface, target, texture, uniforms } from 'vgpu';
...
const drawLogoIcon = (ctx, cx, cy, size) => {
  ctx.save();
  ctx.fillStyle = '#ffffff';

  // Draw bold, crisp lightning emblem
  const s = size * 0.55;
  ctx.beginPath();
  ctx.moveTo(cx + s * 0.12, cy - s * 0.95);
  ctx.lineTo(cx - s * 0.45, cy - s * 0.95);
  ctx.lineTo(cx - s * 0.75, cy + s * 0.05);
  ctx.lineTo(cx - s * 0.05, cy + s * 0.05);
  ctx.lineTo(cx - s * 0.35, cy + s * 0.95);
  ctx.lineTo(cx + s * 0.75, cy - s * 0.12);
...
export default function ShapeWaves({
  ...
  const maskSignature = [text, Boolean(logo), tagline, prompt, fontFamily, fontWeight, textSize].join('|');
    ...
    void (async () => {
      try {
        ...
        const render = now => {
          ...
          const hasAny = Boolean(settings.text.trim() || settings.logo || settings.tagline?.trim() || settings.prompt?.trim());
          params.set({ motion: [drift[0], drift[1], hasAny ? 1 : 0, settings.fade] });
          try {
        ...
        const drawMask = () => {
          ...
          const taglineText = settings.tagline?.trim() ?? '';
          const promptText = settings.prompt?.trim() ?? '';
          const showLogo = Boolean(settings.logo);
          const [surfaceWidth, surfaceHeight] = output.size;
          const hasContent = content.length > 0 || showLogo || taglineText.length > 0 || promptText.length > 0;
          const maskScale = hasContent ? Math.min(1, MAX_MASK_SIZE / Math.max(surfaceWidth, surfaceHeight)) : 0;
          const maskWidth = hasContent ? Math.max(1, Math.round(surfaceWidth * maskScale)) : 1;
          const maskHeight = hasContent ? Math.max(1, Math.round(surfaceHeight * maskScale)) : 1;
          ...
          if (hasContent) {
            ...
            const cx = maskWidth / 2;
            const H = maskHeight;

            if (showLogo && content.length > 0 && promptText.length > 0 && taglineText.length === 0) {
              // Primary mode: Logo + Title ("IssueTrack") + Bold "CLICK TO CONTINUE"
              const logoSize = Math.max(26, Math.round(H * 0.20));
              const logoY = Math.round(H * 0.22);
              const titleY = Math.round(H * 0.50);
              const promptY = Math.round(H * 0.80);

              // 1. Logo
              drawLogoIcon(maskContext, cx, logoY, logoSize);

              // 2. Title ("IssueTrack")
              let titleFontPx = Math.max(1, settings.textSize * H * 0.50);
              maskContext.font = `${settings.fontWeight} ${titleFontPx}px ${settings.fontFamily}`;
              let measured = maskContext.measureText(content).width;
              let maxWidth = maskWidth * 0.88;
              if (measured > maxWidth) {
                titleFontPx = Math.max(1, (titleFontPx * maxWidth) / measured);
                maskContext.font = `${settings.fontWeight} ${titleFontPx}px ${settings.fontFamily}`;
              }
              maskContext.textAlign = 'center';
              maskContext.textBaseline = 'middle';
              maskContext.fillText(content, cx, titleY);

              // 3. Prompt ("CLICK TO CONTINUE") — Bigger, bolder, and more prominent
              let promptFontPx = Math.max(24, Math.round(H * 0.105));
              if ('letterSpacing' in maskContext) {
                maskContext.letterSpacing = '5px';
              }
              maskContext.font = `900 ${promptFontPx}px ${settings.fontFamily}`;
              let pMeasured = maskContext.measureText(promptText).width;
              let pMaxWidth = maskWidth * 0.88;
              if (pMeasured > pMaxWidth) {
                promptFontPx = Math.max(16, (promptFontPx * pMaxWidth) / pMeasured);
                maskContext.font = `900 ${promptFontPx}px ${settings.fontFamily}`;
              }
              maskContext.fillText(promptText, cx, promptY);
              if ('letterSpacing' in maskContext) {
                maskContext.letterSpacing = '0px';
              }
            } else if (showLogo && content.length > 0 && taglineText.length > 0) {
              // Full layout with tagline
              const logoSize = Math.max(22, Math.round(H * 0.16));
              const logoY = Math.round(H * 0.18);
              const titleY = Math.round(H * 0.38);
              const taglineY = Math.round(H * 0.62);
              const promptY = Math.round(H * 0.82);

              drawLogoIcon(maskContext, cx, logoY, logoSize);

              let titleFontPx = Math.max(1, settings.textSize * H * 0.42);
              maskContext.font = `${settings.fontWeight} ${titleFontPx}px ${settings.fontFamily}`;
              let measured = maskContext.measureText(content).width;
              let maxWidth = maskWidth * 0.88;
              if (measured > maxWidth) {
              ...
              let tagFontPx = Math.max(10, Math.round(H * 0.052));
              maskContext.font = `500 ${tagFontPx}px ${settings.fontFamily}`;
              let tagMeasured = maskContext.measureText(taglineText).width;
              let tagMaxWidth = maskWidth * 0.90;
              if (tagMeasured > tagMaxWidth) {
                tagFontPx = Math.max(8, (tagFontPx * tagMaxWidth) / tagMeasured);
                maskContext.font = `500 ${tagFontPx}px ${settings.fontFamily}`;
              }
              maskContext.fillText(taglineText, cx, taglineY);

              if (promptText.length > 0) {
                let promptFontPx = Math.max(16, Math.round(H * 0.065));
                maskContext.font = `800 ${promptFontPx}px ${settings.fontFamily}`;
                let promptMeasured = maskContext.measureText(promptText).width;
                let promptMaxWidth = maskWidth * 0.85;
                if (promptMeasured > promptMaxWidth) {
                  promptFontPx = Math.max(12, (promptFontPx * promptMaxWidth) / promptMeasured);
                  maskContext.font = `800 ${promptFontPx}px ${settings.fontFamily}`;
                }
                maskContext.fillText(promptText, cx, promptY);
              }
            } else if (showLogo && content.length > 0) {
              const logoSize = Math.max(28, Math.round(H * 0.28));
              const logoY = Math.round(H * 0.30);
              const textY = Math.round(H * 0.62);

              drawLogoIcon(maskContext, cx, logoY, logoSize);

              let fontPx = Math.max(1, settings.textSize * H * 0.62);
              maskContext.font = `${settings.fontWeight} ${fontPx}px ${settings.fontFamily}`;
              const measured = maskContext.measureText(content).width;
              const maxWidth = maskWidth * 0.88;
              if (measured > maxWidth) {
              ...
              maskContext.textAlign = 'center';
              maskContext.textBaseline = 'middle';
              maskContext.fillText(content, cx, textY);
            } else if (showLogo) {
              const logoSize = Math.max(32, Math.round(H * 0.45));
              drawLogoIcon(maskContext, cx, H / 2, logoSize);
            } else if (content.length > 0) {
              let fontPx = Math.max(1, settings.textSize * H);
              maskContext.font = `${settings.fontWeight} ${fontPx}px ${settings.fontFamily}`;
              const measured = maskContext.measureText(content).width;
              ...
              maskContext.textAlign = 'center';
              maskContext.textBaseline = 'middle';
              maskContext.fillText(content, cx, H / 2);
            }
          }
